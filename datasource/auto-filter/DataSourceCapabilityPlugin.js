	...
	onBegin: function() {
		...
		DisplayObject.prototype.setupDomObject = function(config) {
			...
			if (endpoint || serviceClass || dsObj) {
				...
				var filterObj = config['datasource-filter-object'];
				var filterEvent = config['datasource-filter-event'] || "keyup";
				if (filterObj) {
					filterObj.addEventListener(filterEvent, function() {
						_self.dispatchEvent('filter', {
							text: filterObj.getValue()
						});
					});
				}
