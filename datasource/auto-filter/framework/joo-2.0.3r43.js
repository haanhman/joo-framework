...
AjaxHandler = {
	...
	handleResponse: function(result, success, fail, url)	{
		if (!result.status)	{
			...
			if (fail != undefined)	{
				try {
					fail.call(undefined, result.status, result.msg);
				} catch (err)	{
					log(err);
				}
			}
...
CompositionRenderInterface = InterfaceImplementor.extend({
	...
	implement: function(obj) {
		 ...
		 */
		obj.prototype.renderUIComposition = obj.prototype.renderUIComposition || function(view, model) {
			model = model || this.config.model || {};
			var composition = $(JOOUtils.tmpl(view || this.className+"View", model));
	...
	processElement: function(root, obj, composition, model) {
		...
		if (composition.nodeType == 3 || composition.nodeType == 8) {	//text node
...
//				bindings = expression;
			...
			(function(i) {
				if (i.indexOf('touch') != -1) {	//in some platforms such as iOS, binding touch events won't take effect
					...
					currentObject.addEventListener('stageUpdated', function() {
						if (currentObject['__hasTouch__'+i]) return;
						currentObject['__hasTouch__'+i] = true;
		...
		if (config['late-value']) {
			currentObject.setValue(config['late-value']);
		}
		if (isVar)
			return undefined;
...
/**
 * @class A counterpart of <code>HTML INPUT HIDDEN</code>
 * @augments JOOInput
 */
JOOHiddenInput = JOOInput.extend({
...
JOOFileInput = JOOInput.extend({
	...
	setupDomObject : function(config) {
		...
		if (config.accept)
			this.setAttribute('accept', config.accept);
	},
...
JOOService = EventDispatcher.extend({
	...
	run: function(params) {
		...
		this.onAjax(this.endpoint, params, this.method, {
			...
			onFailure: function(status, msg) {
				msg = _self.parseError(msg);
				_self.dispatchEvent('failure', {
					status: status,
					msg: msg
				});
...
function printStackTrace(e) {
	...
	if (e.stack) { //Firefox
	    ...
	    for (var i=0, len=lines.length; i<len && i<=10; i++) {
...
ExpressionUtils = {
	...
	express: function(obj, expression) {
		...
		try {
			...
			log("Expression failed: "+err, true);
		}
	},
